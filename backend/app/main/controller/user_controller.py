...
from ..service.user_service import saveNewUser, getAllUsers, getSingleUser,saveNewUsers,getPatients,getDoctors
from ..service.file_service import saveFile,getFile,writeFile,readFile
from ..service.organization_service import isStakeholder
...
api = UserDto.api
_user = UserDto.user 
_patient = UserDto.patient
_doctor = UserDto.doctor
...
class UserList(Resource):
    ...
    def post(self):
        ...
        if(data is not None):
            ...
            _userDto.social_security_number=data['social_security_number']

            response_dto = saveNewUser(_userDto)            
            return  response_dto.toJson(), response_dto.status_code           
             

@api.route('/getPatients')
class PatientList(Resource):
    @api.doc('list_of_available_patients')
...
@api.route('/getDoctors')
class DoctorList(Resource):
    @api.doc('list_of_available_doctors')
    @api.marshal_list_with(_doctor, envelope='data')
    #@jwt_required
    def get(self):
        """List all doctors"""
        return getDoctors()      
...
class RegisterUsers(Resource):
    # Sample Json for data is '{"user_id" : 0 , "stakeholder_id" : 0 }'
    @api.doc('users')
        ...
        if request.files is None or 'file' not in request.files:
            ...
            if((file is not None and data is not None) and file.filename != ''):
                ...
                if(response_object is not None and response_object.status_code == 200):
                    ...
                    stakeholder_id = fileInfoData['stakeholder_id']

                    if(isStakeholder(stakeholder_id)== False):
                        return ''
                    ...
                    if(parsedData is not None):
                        for row in parsedData.rows:                     
                            ...
                            _user.social_security_number=row['Social Security Number']
